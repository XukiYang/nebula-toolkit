...
void General_Fullempty_Testing() {
    ...
    ring_buffer.Write(in);
    LOGP_MSG("IsEmpty:%d,IsFull:%d,Usage:%f%", ring_buffer.IsEmpty() * 1e2, ring_buffer.IsFull() * 100,
             ring_buffer.Usage());
    ring_buffer.Read(out, in.size());
    LOGP_MSG("IsEmpty:%d,IsFull:%d,Usage:%f%", ring_buffer.IsEmpty() * 1e2, ring_buffer.IsFull() * 100,
             ring_buffer.Usage());
...
void General_IO_CustTime_Testing() {
    ...
    for (size_t i = 0; i < measurement_count; ++i) {
        ...
        auto write_ns  = std::chrono::duration_cast<std::chrono::nanoseconds>(write_end - write_start).count();
