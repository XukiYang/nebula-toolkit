...
private:
    ...
    void CustThreadProc() {
        ...
        while (cust_thread_running_.load()) {
            ...
            if (cv.wait_for(lock, std::chrono::milliseconds(100), [&] { return !ring_buffer_->IsEmpty(); })) {
                // 限制最大块字节，选取最小可读字节
                size_t available_to_read = ring_buffer_->AvailableToRead();
                size_t min_read_bytes    = std::min(available_to_read, log_async_config_.batch_size_kb);

                if (min_read_bytes > 0) {
                    ring_buffer_->Read(read_buffer, min_read_bytes);
                    RotateFileIfNeeded();
                    file_manager_.file.write(reinterpret_cast<const char*>(read_buffer.data()),
