...
public:
    /// @brief 指定线程数构造线程
    /// @param thread_count
    explicit ThreadPool(size_t thread_count = std::thread::hardware_concurrency()) : running_(true) {
    ...
    void WorkerLoop() {
        while (true) {
            CallBack cb_task;
            {
                ...
                cv_.wait(lock, [this] { return !running_.load(std::memory_order_acquire) || !task_queue_.empty(); });
