...
private:
    ...
    UnPacker(HeadKey&& head_key, TailKey&& tail_key, size_t buffer_size = 1024)
        : RingBuffer(buffer_size), head_key_(std::move(head_key)), tail_key_(std::move(tail_key)) {
    ...
    /// @param check_sz_cb
    /// @param buffer_size
    UnPacker(HeadKey&& head_key, TailKey&& tail_key, DataSzCb&& data_sz_cb, CheckValidCb&& check_sz_cb,
...
private:
    ...
    /// @param read_data
    /// @return UnPackerResult
    UnPackerResult ProcessHeadOnlyMode(std::vector<std::vector<uint8_t>>& read_data) {
    ...
    /// @param read_data
    /// @return UnPackerResult
    UnPackerResult ProcessHeadTailMode(std::vector<std::vector<uint8_t>>& read_data) {
        size_t       current_pos = 0;
        const size_t total_size  = AvailableToRead();  // 保存当前可读数据总量
        while (current_pos < total_size) {
            // 查找头定位符
    ...
    /// @param read_data
    /// @return UnPackerResult
    UnPackerResult ProcessHeadTailAndCbMode(std::vector<std::vector<uint8_t>>& read_data) {
        size_t       current_pos = 0;
        const size_t total_size  = AvailableToRead();
