...
public:
    ...
    size_t Write(const std::vector<T>& write_data) {
        ...
        const size_t first_chunk = std::min(write_data_size, buffer_.size() - write_index_);

        memcpy(buffer_.data() + write_index_, write_data.data(), first_chunk);
        if (write_data_size > first_chunk) {
            memcpy(buffer_.data(), write_data.data() + first_chunk, write_data_size - first_chunk);
        }
    ...
    size_t Read(std::vector<T>& read_data, size_t bytes_to_read) {
        ...
        if (bytes_to_read > first_chunk) {
            memcpy(read_data.data() + (first_chunk / sizeof(T)), buffer_.data(), bytes_to_read - first_chunk);
        }
    ...
    size_t Write(const std::byte* write_ptr, size_t bytes_to_write) {
        ...
        const size_t first_chunk = std::min(bytes_to_write, buffer_.size() - write_index_);

        memcpy(buffer_.data() + write_index_, write_ptr, first_chunk);
        if (bytes_to_write > first_chunk) {
            memcpy(buffer_.data(), write_ptr + first_chunk, bytes_to_write - first_chunk);
        }
...
public:
    ...
    /// @return 使用率
    float Usage() const {
        return static_cast<float>(length_) / buffer_.size();
    }
...
public:
    ...
    std::pair<uint8_t*, size_t> GetLinearWriteSpace() {
        ...
        size_t linear_space = std::min(AvailableToWrite(), buffer_.size() - write_index_);
    ...
    std::pair<const uint8_t*, size_t> GetLinearReadSpace() {
        ...
        size_t linear_space = std::min(AvailableToRead(), buffer_.size() - read_index_);
